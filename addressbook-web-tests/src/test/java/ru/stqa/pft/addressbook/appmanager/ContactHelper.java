...
public class ContactHelper extends HelperBase {
    ...
    public Contacts all() {
        ...
        for (WebElement element : elements) {
            ...
            String email = names.get(4).getText();
            String allPhones = names.get(5).getText();
            int id = Integer.parseInt(element.findElement(By.tagName("input")).getAttribute("value"));
            contacts.add(new ContactData().withId(id).withFirstname(firstname).withLastname(lastname).withAddress(address).withEmail(email).withAllPhones(allPhones));
        }
        return contacts;
    }
    ...
    public ContactData infoFromEditForm(ContactData contact) {
        ...
        wd.navigate().back();
        return  new ContactData().withId(contact.getId()).withFirstname(firstname).withLastname(lastname).withAddress(address).withEmail(email).withHome(home).withMobile(mobile).withWork(work);
    }
}
